function y = plotRegressionBounds(Ddata) %input a regression struct
reg = Ddata.reg;
...
xlabel('log10(1/L)');
ylabel('log10(N)');
title(['Name:' name ' min Pts:' num2str(reg.minPtsAllowed) ' FSCO =' num2str(reg.FSCOBS) 'um , LSCO =' num2str(reg.LSCOBS) 'um , D =' num2str(reg.D)]);
