...
function  [Vnew, VAllnew] = straightenNeuronP(V, VAll, DM, SLA, WL, PL, percent)
    Vnew = V;
    VAllnew = VAll; %--
    for i=1:size(SLA,1)
...
%             [newSeg,Vnew,newSegAll,VAllnew] = rotateNeuronSegAllAngle(Vnew,VAllnew,DM,SLA,segV,segVAll,segVparent,i,percent);%changes even the very first angle
             [newSeg,Vnew,newSegAll,VAllnew] = rotateNeuronSegP(Vnew,VAllnew,DM,SLA,segV,segVAll,segVparent,i,percent);%changes only the very first angle
