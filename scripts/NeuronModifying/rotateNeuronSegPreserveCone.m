...
function [newSeg,Vnew,newSegAll,VAllnew] = rotateNeuronSegPreserveCone(Vnew,VAllnew,DM,SLA,segV,segVAll,segN,percent)
    ...
    segVend = segV(end,:);
    for i=(size(segV,1)-1):-1:1 % if you end at 1 then it reorients the branch towards the end.
...
%         scatter3(segVAll(:,1),segVAll(:,2),segVAll(:,3),'filled')
        [segV,segVAll,eVector, theta, translationPT] = rotateNeuronVectorsPreserveCone(segV,segVAll,segVend,percent,i);
