...
function masterN = bNCt(readPath,writePathBase,Range,method,varargin)
...
for i=1:length(masterN)%length(masterN):-1:1%198:-1:100%length(masterN):-1:150
        ...
        if (exist(fullfile(writePath,[masterN(i).name '-' num2str(Range) '.mat']),'file') == 2) && ~p.Results.redo
            neuron = load(fullfile(writePath,[masterN(i).name '-' num2str(Range) '.mat']));
...
%             try
                ...
                [V,VAll,SLA,VRM,DM,WL,PL,IL,N,Face]=pNR(masterN(i).name,masterN(i).path,writePath,Range,scaling,method);
                masterN(i).data = getCurveData2(Range,V,VAll,N,method); %get data specific to each curvature
    ...
    %                 specificN = masterN(i);
                ...
                parsave(fullfile(writePath,[masterN(i).name '-' num2str(Range) '.mat']),masterN(i));
                if p.Results.timeParts
                    e = etime(clock,t);
