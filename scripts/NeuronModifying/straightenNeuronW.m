...
function  [Vnew, VAllnew] = straightenNeuronW(V, VAll, DM, SLA, WL, percent)
    Vnew = V;
    VAllnew = VAll; %--
    for i=1:size(SLA,1)
...
%             [newSeg,Vnew] = rotateNeuronSeg(Vnew,DM,SLA,segV,i,percent);
            [newSeg,Vnew,newSegAll,VAllnew] = rotateNeuronSegW(Vnew,VAllnew,DM,SLA,segV,segVAll,i,percent);%keeps original angle            
