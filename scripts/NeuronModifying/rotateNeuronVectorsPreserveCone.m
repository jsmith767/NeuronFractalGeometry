...
function [newVector,newVectorAll,eVector,theta, translationPT] = rotateNeuronVectorsPreserveCone(verts,vertsAll,vertsEnd,percent,junctionNum) 
        %determine the angle between the first segment and the line from
        %the first point to the last.  
        if (junctionNum == 1)
            theta = real(acos(dot(verts(end,1:3)-verts(1,1:3),vertsEnd(1:3)-verts(1,1:3))...
            /(norm(verts(end,1:3)-verts(1,1:3))*norm(vertsEnd(1:3)-verts(1,1:3)))));
        ...
        if (junctionNum == 1)
            eVector = cross(vertsEnd(1:3)-verts(1,1:3),verts(end,1:3)-verts(1,1:3));
        else
