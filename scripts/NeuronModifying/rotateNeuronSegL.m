...
function [newSeg,Vnew,newSegAll,VAllnew] = rotateNeuronSegL(Vnew,VAllnew,DM,SLA,segV,segVAll,segVparent,segN,factor)
...
%         ind = 1;
%     end
    for i=(size(segV,1)-1):-1:1 %runs through segments of the current branch from end to start
        [segV,segVAll,eVector, theta, translationPT] = rotateNeuronVectorsL(segV,segVAll,segValign,factor,i);
        segV = segV*factor;
        segVAll = segVAll*factor;
        translationPTV = segV(end,1:3);
        translationPTVAll = segVAll(end,1:3);
%         segRingsToRot = segVAll(i*16+1:end,:);%--
%         segVAll = rotateSegByAngle(segRingsToRot,eVector,theta,translationPT);%--
        
        for j=2:(1+DM(segN,1)) %runs through daughter branches of the current segment that this pt is in
            ...
            rotatedSeg = rotateSegByAngle(segToRot,eVector,theta,translationPTV);
            rotatedSeg = rotatedSeg(:,1:3);
            rotatedSegAll = rotateSegByAngle(segRingsToRot,eVector,theta,translationPTVAll);%--
