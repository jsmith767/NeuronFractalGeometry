...
function  [Vnew, VAllnew] = straightenNeuronSA(V, VAll, DM, SLA, WL, PL, thetaFixed)
    Vnew = V;
    VAllnew = VAll; %--
    for i=1:size(SLA,1)
...
%             [newSeg,Vnew,newSegAll,VAllnew] = rotateNeuronSegAllAngle(Vnew,VAllnew,DM,SLA,segV,segVAll,segVparent,i,percent);%changes even the very first angle
             [newSeg,Vnew,newSegAll,VAllnew] = rotateNeuronSegSA(Vnew,VAllnew,DM,SLA,segV,segVAll,segVparent,i,thetaFixed);%changes only the very first angle
