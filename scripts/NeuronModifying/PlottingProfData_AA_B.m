...
BNumMods = size(plottingN(1).data.(genvarname(method)),2);
...
for c = 1:BNumMods
    for n = 1:size(plottingN,2)
        if ~isempty(plottingN(n).data)
            if ~isempty(plottingN(n).data.(genvarname(method))(c).Ddata)
                Dcell{c,n} = plottingN(n).data.(genvarname(method))(c).Ddata.reg.D;
                Vcell{c,n} = plottingN(n).data.(genvarname(method))(c).volume;
                Pcell{c,n} = plottingN(n).data.(genvarname(method))(c).AveP;
                Mcell{c,n} = plottingN(n).data.(genvarname(method))(c).mass;
                SAcell{c,n} = plottingN(n).data.(genvarname(method))(c).SA;
                SAtcell{c,n} = plottingN(n).data.(genvarname(method))(c).SAt;
            end
        end
    end
end
...
plotrangevol = [0 9];
figure('Name','2','units','normalized')
ax2P = subplot(2,1,1);
hold(ax2P,'on');
...
figure('Name','3','units','normalized')
ax3P = subplot(2,1,1);
hold(ax3P,'on');
...
plotrangemass = [0 6];
figure('Name','4','units','normalized')
...
figure('Name','7','units','normalized')
...
% This figure only shows the natural neurons
figure('Name','6','units','normalized')
...
figure('Name','9','units','normalized')
...
% This figure only shows the natural neurons
figure('Name','8','units','normalized')
...
if ShowSANormPlots == 1
    binRange = [1.32 1.72];
    Numbins = 9;
    figure('Name','15','units','normalized')
...
% % %     figure('Name','16','units','normalized')
...
% neurons where connectivity is normalized by surface area.
    ...
    figure('Name','14','units','normalized')
    ax14P = subplot(1,4,1);
    hold(ax14P,'on');
...
if ShowBoxPlots == 1
    figure('Name','10','units','normalized')
    ax10P = subplot(2,1,1);
    hold(ax10P,'on');
    ...
    figure('Name','11','units','normalized')
    ax11P = subplot(2,1,1);
    hold(ax11P,'on');
    ...
    figure('Name','12','units','normalized')
    ax12P = subplot(2,1,1);
    hold(ax12P,'on');
    ...
    figure('Name','13','units','normalized')
    ax13P = subplot(2,1,1);
    hold(ax13P,'on');
...
% clearvars -except plottingN D V P M L SA SAt BFbyM BDbyM BVbyM BPbyM BMbyM BLbyM BSAbyM BSAtbyM BNumMods order Corder clr PlotFont BoxPlotFont LabelFont TitleFont