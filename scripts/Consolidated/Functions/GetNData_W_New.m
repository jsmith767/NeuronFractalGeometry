function [D,V,P,BA,M,SA,SAB,VM,R,L,BDbyM,BVbyM,BPbyM,BBAbyM,BMbyM,BSAbyM,BSABbyM,BVMbyM,BRbyM,BLbyM,BFbyM] = GetNData_W_New(plottingN)
        ...
        for n = 1:size(plottingN,2)
            if ~isempty(plottingN(n).data)
                if ~isempty(plottingN(n).data.W(c).D)
                    ...
                    BAcell{c,n} = plottingN(n).data.W(c).AveBA;
