function [p,plist,pit,pilist,latlonlist] = ProfileAreaByIteration(structure,viewpoints,D)
...
N = zeros(voxcount,3);
iterations = zeros(voxcount,1);
voxcount = 0;
for i = 1:itcount
    for j = 1:size(structure(i).voxUAll,1)
        voxcount = voxcount + 1;
        N(voxcount,:) = structure(i).voxUAll(j,:);
...
for i = 1:viewpoints
    ...
    RM = round(horzcat(Mview,iterations));
