function [D,V,P,BA,M,SA,SAB,VM,R,L,BDbyM,BVbyM,BPbyM,BBAbyM,BMbyM,BSAbyM,BSABbyM,BVMbyM,BRbyM,BLbyM,BFbyM] = GetNData_P_New(plottingN)
    ...
    for c = 1:BNumMods
        for n = 1:size(plottingN,2)
            if ~isempty(plottingN(n).data)
                if ~isempty(plottingN(n).data.P(c).D)
                    Dcell{c,n} = plottingN(n).data.P(c).D;
                    Vcell{c,n} = plottingN(n).data.P(c).volume;
                    Pcell{c,n} = plottingN(n).data.P(c).AveP;
                    BAcell{c,n} = plottingN(n).data.P(c).AveBA;
                    Mcell{c,n} = plottingN(n).data.P(c).mass;
                    SAcell{c,n} = plottingN(n).data.P(c).SA;
                    SABcell{c,n} = plottingN(n).data.P(c).convHullSA;
                    VMcell{c,n} = plottingN(n).data.P(c).mass;
                    Rcell{c,n} = plottingN(n).data.P(c).ArbR;
                end
            end
        end
    end
