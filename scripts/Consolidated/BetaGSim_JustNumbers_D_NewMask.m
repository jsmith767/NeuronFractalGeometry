...
UseGauss = 0;
MakeEucMask = 0;
CellRad = Rads(1,Rad); %39 (~25um) or 31 (~20um) for 77 pix width (~25um) rows
NumSamps = 100;
QuarterGlia = floor(pi*CellRad^2/4);

if UseMask == 1
%     mask = imread('K:\Keck\MicroscopeImages\BP1\Pluth\BP1_20x_CNT\FullImage\mask.tif');%for D = 1.1
    mask = im2uint8(imread('K:\Keck\Programs\Matlab_Julian_10-30-2018\Templates\D1_1-I4-W25-FL2163_pad.tif'));%for D = 1.1
    savefilename = strcat('D1.1-BetaGSim-',num2str(NumSamps),'Samples-R',num2str(CellRad),'_pad');
elseif UseMask == 2
%     mask = imread('K:\Keck\MicroscopeImages\BP10\Pluth\BP10_20x_CNT\FullImage\mask.tif');%for D = 1.5
    mask = im2uint8(imread('K:\Keck\Programs\Matlab_Julian_10-30-2018\Templates\D1_5-I4-W25-FL1855_pad.tif'));%for D = 1.5
    savefilename = strcat('D1.5-BetaGSim-',num2str(NumSamps),'Samples-R',num2str(CellRad),'_pad');
elseif UseMask == 3
%     mask = imread('K:\Keck\MicroscopeImages\BO1-1\Pluth\BO1-1_20x_CNT\FullSample\mask.tif');%for D = 2, I = 4
    mask = im2uint8(imread('K:\Keck\Programs\Matlab_Julian_10-30-2018\Templates\D2-I4-W25-FL1600_pad.tif'));%for D = 2, I = 4
    savefilename = strcat('D2.0-BetaGSim-',num2str(NumSamps),'Samples-R',num2str(CellRad),'_pad');
elseif UseMask == 4
%     mask = imread('K:\Keck\MicroscopeImages\BO1-1\Pluth\BO1-1_20x_CNT\FullSample\mask.tif');%for D = 2, I = 4
    mask = im2uint8(imread('K:\Keck\Programs\Matlab_Julian_10-30-2018\Templates\D2-I4-W25-FL1664_pad.tif'));%for D = 2, I = 4
    savefilename = strcat('D2.0I4-BetaGSim-',num2str(NumSamps),'Samples-R',num2str(CellRad),'_pad');
elseif UseMask == 5
%     mask = imread('K:\Keck\MicroscopeImages\BK1-1\Pluth\BK1-1_20x_CNT\FullSample\mask.tif');%for D = 2, I = 5
    mask = im2uint8(imread('K:\Keck\Programs\Matlab_Julian_10-30-2018\Templates\D2-I5-W25-FL1611_pad.tif'));%for D = 2, I = 5
    savefilename = strcat('D2.0I5-BetaGSim-',num2str(NumSamps),'Samples-R',num2str(CellRad),'_pad');
elseif UseMask == 6
%     mask = imread('K:\Keck\MicroscopeImages\BP11\Pluth\BP11_20x_CNT\FullImage\mask.tif');%for D = 2, I = 6
    mask = im2uint8(imread('K:\Keck\Programs\Matlab_Julian_10-30-2018\Templates\D2-I6-W25-FL1585_pad.tif'));%for D = 2, I = 6
    savefilename = strcat('D2.0I6-BetaGSim-',num2str(NumSamps),'Samples-R',num2str(CellRad),'_pad');
end
...
if MultCellNum == 1
    CellDens = [5 10 20 50 100 500]./250000000;
    NumCells = round(CellDens.*(Xlims*Ylims));
else
    CellDens = 100/250000000;
    NumCells = round(CellDens.*(Xlims*Ylims));
end
...
clearvars -except plottingN plottingH masterN masterH HTreeM mNVox Rad Rads
