% load('K:\Keck\Programs\masterN\MatlabBoxCountingMethod\masterNOM1_2-oneFifth_A_Apical.mat');
...
%         end
%         [p,plist,pit,pilist,latlonlist] = ProfileAreaByIteration(masterN(n).data.W(c).ItData,viewpoints,ExpandDiameterBy+1);
...
%         end
%         [p,plist,pit,pilist,latlonlist] = ProfileAreaByIteration(masterN(n).data.I(c).ItData,viewpoints,ExpandDiameterBy+1);
...
%         end
%         [p,plist,pit,pilist,latlonlist] = ProfileAreaByIteration(masterN(n).data.original(c).ItData,viewpoints,ExpandDiameterBy+1);
...
%         end
%         [p,plist,pit,pilist,latlonlist] = ProfileAreaByIteration(masterN(n).data.init(c).ItData,viewpoints,ExpandDiameterBy+1);
...
%         end
        [p,plist,pit,pilist,latlonlist] = ProfileAreaByIteration(masterN(n).data.allangles(c).ItData,viewpoints,ExpandDiameterBy+1);
...
save('K:\Keck\Programs\masterN\MatlabBoxCountingMethod\masterNOM1_2-oneFifth_A_Apical_Prof.mat','masterN','-v7.3');
clear('masterN')